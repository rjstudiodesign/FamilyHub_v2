import { db, collection, query, onSnapshot, where, addDoc, serverTimestamp, doc, updateDoc, deleteDoc } from './firebase.js';
      ...
      export function renderWishlist(listeners) {
              ...
              const modalContent = `
                <div class="modal-content glass-premium max-w-lg w-full">
                    ...
                    <form id="create-wish-form" class="space-y-4">
                        <div>
                            <label for="wish-url" class="form-label">Link zum Produkt (optional)</label>
                            <input type="url" id="wish-url" class="form-input" placeholder="https://beispiel.de/produkt">
                            <p class="text-xs text-secondary mt-1">Füge einen Link ein, um Details automatisch auszufüllen.</p>
                        </div>
                        <div>
                            <label for="wish-title" class="form-label">Was wünschst du dir?</label>
                            <input type="text" id="wish-title" class="form-input" required>
                        </div>
                        <div>
                            <label for="wish-price" class="form-label">Preis (optional)</label>
                            <input type="text" id="wish-price" class="form-input" placeholder="z.B. 29,99 €">
                        </div>
                        <div>
                            <label for="wish-description" class="form-label">Beschreibung (optional)</label>
                            <textarea id="wish-description" class="form-input" rows="3"></textarea>
                        </div>
          ...
          window.handleWishSubmit = async (event) => {
              ...
              const newWish = {
                  title: title,
                  url: document.getElementById('wish-url').value || null,
                  price: document.getElementById('wish-price').value || null,
                  description: document.getElementById('wish-description').value || null,
              ...
              try {
                  ...
                  showNotification("Wunsch hinzugefügt!", "success");
