...
function formatCurrency(amount) {
    ...
    const modalContent = `
        <form id="create-expense-form">
            <h2 class="text-2xl font-bold text-gradient mb-6">Neue Ausgabe</h2>
...
async function handleExpenseSubmit(e) {
    ...
    try {
        ...
        // --- ÄNDERUNG HIER ---
        // const postsCol = collection(db, 'families', currentFamilyId, 'posts'); // ALT
        const expensesCol = collection(db, 'families', currentFamilyId, 'expenses'); // NEU
        
        // await addDoc(postsCol, { // ALT
        await addDoc(expensesCol, { // NEU
            // type: 'expense', // Nicht mehr nötig, da eigene Sammlung
            text: text,
            amount: amount,
            ...
            createdAt: serverTimestamp(),
            // likes: [], // Nicht relevant für Ausgaben
            // commentsCount: 0 // Nicht relevant für Ausgaben
        });
